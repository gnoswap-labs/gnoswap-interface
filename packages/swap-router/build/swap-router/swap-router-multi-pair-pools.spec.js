...
const multiPairPools = (0, mapper_1.makePoolsByRPC)([
    {
...
describe("swap router of multi pair pools", () => {
    describe("gno.land/r/bar to gno.land/r/qux", () => {
        test("EXANCT_IN, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(multiPairPools);
            const estimatedRoutes = swapRouter.estimateSwapRoute("gno.land/r/bar", "gno.land/r/qux", 10000n, "EXACT_IN");
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(2);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:500*POOL*gno.land/r/baz:gno.land/r/qux:500");
            expect(estimatedRoutes[0].quote).toBe(95);
            expect(estimatedRoutes[1].routeKey).toBe("gno.land/r/bar:gno.land/r/foo:500*POOL*gno.land/r/foo:gno.land/r/qux:500");
            expect(estimatedRoutes[1].quote).toBe(5);
            expect(sumAmount).toBe(39840n);
        });
        test("EXACT_OUT, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(multiPairPools);
            const estimatedRoutes = swapRouter.estimateSwapRoute("gno.land/r/bar", "gno.land/r/qux", 10000n, "EXACT_OUT");
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(1);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/bar:gno.land/r/foo:500*POOL*gno.land/r/foo:gno.land/r/qux:500");
            expect(estimatedRoutes[0].quote).toBe(100);
            expect(sumAmount).toBe(778n);
        });
    });
    describe("gno.land/r/qux to gno.land/r/bar", () => {
        test("EXANCT_IN, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(multiPairPools);
            const estimatedRoutes = swapRouter.estimateSwapRoute("gno.land/r/qux", "gno.land/r/bar", 10000n, "EXACT_IN");
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(1);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/baz:gno.land/r/qux:500*POOL*gno.land/r/bar:gno.land/r/baz:500");
            expect(estimatedRoutes[0].quote).toBe(100);
            expect(sumAmount).toBe(2428n);
        });
        test("EXACT_OUT, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(multiPairPools);
            const estimatedRoutes = swapRouter.estimateSwapRoute("gno.land/r/qux", "gno.land/r/bar", 10000n, "EXACT_OUT");
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(1);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/foo:gno.land/r/qux:500*POOL*gno.land/r/bar:gno.land/r/foo:500");
            expect(estimatedRoutes[0].quote).toBe(100);
            expect(sumAmount).toBe(5527n);
        });
    });
});
