...
const singlePairPools = (0, mapper_1.makePoolsByRPC)([
    {
...
const singlePairMultiPools = (0, mapper_1.makePoolsByRPC)([
    {
...
describe("swap router of single pair pool", () => {
    describe("gno.land/r/bar to gno.land/r/baz", () => {
        test("EXANCT_IN, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(singlePairPools);
...
describe("swap router of single pair multi pools", () => {
    describe("gno.land/r/bar to gno.land/r/baz", () => {
        test("EXANCT_IN, 100000", async () => {
            const swapRouter = new swap_router_1.SwapRouter(singlePairMultiPools);
            const estimatedRoutes = swapRouter.estimateSwapRoute("gno.land/r/bar", "gno.land/r/baz", 10000n, "EXACT_IN");
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(3);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:500");
            expect(estimatedRoutes[0].quote).toBe(60);
            expect(estimatedRoutes[1].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:100");
            expect(estimatedRoutes[1].quote).toBe(35);
            expect(estimatedRoutes[2].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:3000");
        ...
        test("EXACT_OUT, 100000", async () => {
            ...
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(1);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:3000");
            expect(estimatedRoutes[0].quote).toBe(100);
    ...
    describe("gno.land/r/baz to gno.land/r/bar", () => {
        test("EXANCT_IN, 100000", async () => {
            ...
            const sumAmount = (0, array_util_1.sumBigInts)(estimatedRoutes.map(route => route.amountOut));
            expect(estimatedRoutes).toHaveLength(1);
            expect(estimatedRoutes[0].routeKey).toBe("gno.land/r/bar:gno.land/r/baz:100");
            expect(estimatedRoutes[0].quote).toBe(100);
