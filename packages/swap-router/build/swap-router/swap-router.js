...
exports.SwapRouter = exports.TOP_N_WITH_BASE_TOKEN = exports.TOP_N_WITH_EACH_BASE_TOKEN = exports.TOP_N_SECOND_HOP = exports.TOP_N_TOKEN_IN_OUT = exports.TOP_N_DIRECT_SWAPS = exports.TOP_N = void 0;
const bignumber_js_1 = __importDefault(require("bignumber.js"));
const common_1 = require("../common");
const array_util_1 = require("../common/array.util");
const swap_simulator_1 = require("../swap-simulator");
const route_util_1 = require("./utility/route.util");
const constants_1 = require("../constants");
exports.TOP_N = 2;
exports.TOP_N_DIRECT_SWAPS = 2;
exports.TOP_N_TOKEN_IN_OUT = 3;
exports.TOP_N_SECOND_HOP = 1;
exports.TOP_N_WITH_EACH_BASE_TOKEN = 3;
exports.TOP_N_WITH_BASE_TOKEN = 5;
class SwapRouter {
    constructor(pools) {
        this.estimateSwapRoute = (inputTokenPath, outputTokenPath, amount, exactType, distributionRatio = 5, hopSize = 7) => {
            ...
            const routes = this.findCandidateRoutesBy(this.pools, inputTokenPath, outputTokenPath);
            const filteredRoutes = routes.filter((_, index) => index < hopSize);
            const simulator = new swap_simulator_1.SwapSimulator();
            /**
             * Store the results of the swap route by input token amount.
             */
            const routeWithQuotes = {};
            for (const route of filteredRoutes) {
                const routeKey = (0, route_util_1.makeRouteKey)(route);
                for (let ratio = 100; ratio >= 0; ratio -= currentDistributionRatio) {
                    try {
                        let currentInputTokenPath = inputTokenPath;
                        let currentAmount = BigInt((0, bignumber_js_1.default)(amount.toString())
                        ...
                        for (const pool of route.pools) {
                            ...
                            const { amountA, amountB } = simulator.swap(pool, currentInputTokenPath, currentAmount, exactType);
                        ...
                        const amountRatio = exactType === "EXACT_IN"
            ...
            const quoteMap = Object.keys(routeWithQuotes).reduce((accum, key) => {
             ...
             */
            let quoteSum = (0, array_util_1.sum)(Object.values(quoteMap).map(({ quote }) => quote));
            while (quoteSum > 100) {
                let decreaseTargetKey = null;
                let sumAmount = null;
                for (const routeKey of Object.keys(quoteMap)) {
                    if (quoteMap[routeKey].quote <= 0) {
                        continue;
                    }
                    const nextRoute = routeWithQuotes[routeKey].find(route => route.quote === quoteMap[routeKey].quote - currentDistributionRatio);
                    if (!nextRoute) {
                        continue;
                    }
                    const currentSumOfAmmountOut = Object.entries(quoteMap).reduce((accum, [key, value]) => {
                        return key === routeKey
                ...
                if (decreaseTargetKey && quoteMap[decreaseTargetKey]) {
                    const changedQuote = quoteMap[decreaseTargetKey].quote - currentDistributionRatio;
                    const amountOut = routeWithQuotes[decreaseTargetKey].find(route => route.routeKey === decreaseTargetKey &&
                        route.quote === changedQuote)?.amountOut;
                    quoteMap[decreaseTargetKey] = {
                        quote: changedQuote > 0 ? changedQuote : 0,
                        amountOut: amountOut ?? 0n,
                    };
                }
                quoteSum = (0, array_util_1.sum)(Object.values(quoteMap).map(({ quote }) => quote));
            }
            /**
            ...
            Object.entries(quoteMap)
                ...
                const route = routeWithQuotes[routeKey].find(route => route.quote === value.quote);
                if (route) {
            ...
            return estimatedRoutes.sort((route1, route2) => route2.quote - route1.quote);
        };
        this._pools = pools;
    }
    get pools() {
    ...
    filteredCandidatePools(pools, inputTokenPath, outputTokenPath) {
        const availPools = pools.filter(pool => pool.liquidity > 0);
        const topN = 10;
        const topByBaseWithTokenIn = availPools
            .filter(p => p.tokenAPath === inputTokenPath || p.tokenBPath === inputTokenPath)
            .sort((p1, p2) => {
            const priceOfP1 = p1.tokenAPath === inputTokenPath ? p1.price : 1 / p1.price;
            const priceOfP2 = p2.tokenAPath === inputTokenPath ? p2.price : 1 / p2.price;
            return priceOfP2 - priceOfP1;
        })
            .filter((_, index) => index < exports.TOP_N_WITH_EACH_BASE_TOKEN);
        const topByBaseWithTokenOut = availPools
            .filter(p => p.tokenAPath === outputTokenPath || p.tokenBPath === outputTokenPath)
            .sort((p1, p2) => {
            const priceOfP1 = p1.tokenAPath === outputTokenPath ? 1 / p1.price : p1.price;
            const priceOfP2 = p2.tokenAPath === outputTokenPath ? 1 / p2.price : p2.price;
            return priceOfP2 - priceOfP1;
        })
            .filter((_, index) => index < exports.TOP_N_WITH_EACH_BASE_TOKEN);
        const topByDirectSwapPool = availPools
            .filter(p => [inputTokenPath, outputTokenPath].includes(p.tokenAPath) ||
            [inputTokenPath, outputTokenPath].includes(p.tokenBPath))
            .filter((_, index) => index < exports.TOP_N_DIRECT_SWAPS);
        const topByTVL = availPools
            .sort((p1, p2) => Number(p2.liquidity - p1.liquidity))
            .filter((_, index) => index < exports.TOP_N);
        return {
    ...
    findCandidateRoutesBy(pools, inputTokenPath, outputTokenPath, routeSize = 3) {
        function getPoolTokenPaths(pool) {
            return [pool.tokenAPath, pool.tokenBPath];
        }
        function getOtherTokenPath(pool, tokenPath) {
            return pool.tokenAPath === tokenPath ? pool.tokenBPath : pool.tokenAPath;
        }
        const routes = [];
        const { topByBaseWithTokenIn, topByBaseWithTokenOut, topByDirectSwapPool, topByTVL, } = this.filteredCandidatePools(pools, inputTokenPath, outputTokenPath);
        ...
        // Pool list of tokens
        const tokenPoolsMap = candidatePools.reduce((result, current) => {
        ...
        const routeQueue = new common_1.Queue();
        // Initialize queue
        const initialPools = tokenPoolsMap[inputTokenPath];
        initialPools.forEach(pool => routeQueue.enqueue({
        ...
        while (routeQueue.arr.length !== 0) {
            ...
            for (let index = 0; index < queueItems.length; index++) {
                ...
                const nextPools = tokenPoolsMap[current.lastTokenPath].filter(({ tokenAPath, tokenBPath }) => !tokenPathsOfRoute.includes(tokenAPath) &&
                    !tokenPathsOfRoute.includes(tokenBPath));
                // Add a queue item
                for (const nextPool of nextPools) {
                    const nextLastTokenPath = getOtherTokenPath(nextPool, current.lastTokenPath);
        ...
        function getResultPriceRate(pools) {
        ...
        function getResultMinLiquidity(pools) {
            let minLiquidity = constants_1.MAX_UINT64;
            pools.forEach(pool => {
        ...
        const poolPathSet = new Set([]);
        const uniqueSortedRoutes = sortedRoutes.filter(route1 => {
            const exists = route1.pools.findIndex(pool => poolPathSet.has(pool.poolPath)) > -1;
