...
import { ChainId, CurrencyAmount, Gnot, Price } from "../../core";
...
export class V3HeuristicGasModelFactory extends IOnChainGasModelFactory {
  ...
  > {
    ...
    const estimateGasCost = (
      ...
      /** TODO: Gno gas model is unclear */
      if (!usdPool || !usdToken) {
        return {
          gasEstimate: baseGasUse,
          gasCostInToken: totalGasCostNativeCurrency,
          gasCostInUSD: totalGasCostNativeCurrency,
          gasCostInGasToken: totalGasCostNativeCurrency,
        };
      }
  ...
  private estimateGas(
    ...
    const totalInitializedTicksCrossed = BigNumber(
      Math.max(1, _.sum(routeWithValidQuote.initializedTicksCrossedList)),
    );
    const totalHops = BigNumber(routeWithValidQuote.route.pools.length);

    let hopsGasUse = COST_PER_HOP(chainId).multipliedBy(totalHops);
    ...
    if (totalHops.eq(1)) {
      hopsGasUse = hopsGasUse.plus(SINGLE_HOP_OVERHEAD(chainId));
    }
    ...
    const tickGasUse = COST_PER_INIT_TICK(chainId).multipliedBy(
      totalInitializedTicksCrossed,
    );
    const uninitializedTickGasUse = COST_PER_UNINIT_TICK.multipliedBy(0);
    ...
    const baseGasUse = BASE_SWAP_COST(chainId)
      ...
      .plus(tokenOverhead)
      .plus(tickGasUse)
      .plus(uninitializedTickGasUse)
      .plus(providerConfig?.additionalGasOverhead ?? BigNumber(0));

    const baseGasCostWei = gasPriceWei.multipliedBy(baseGasUse);
