...
import { WGNOT_DEV } from "../../test-utils/mock-token";
...
describe("Pool", () => {
  ...
  describe("constructor", () => {
    ...
    it("fee cannot be more than 1e6", () => {
      expect(() => {
        new Pool(USDC, WGNOT_DEV, 1e6, encodeSqrtRatioX96(1, 1), 0, 0, []);
      }).toThrow("FEE");
    });
  ...
  describe("#priceOf", () => {
    ...
    it("throws if invalid token", () => {
      expect(() => pool.priceOf(WGNOT_DEV)).toThrow("TOKEN");
    });
  });
  ...
  describe("#involvesToken", () => {
    ...
    expect(pool.involvesToken(WGNOT_DEV)).toEqual(false);
  });
