...
import { useGetChainInfo, useGetTokenDetails, useGetTokens } from "@query/token";
...
const GainerAndLoserContainer: React.FC = () => {
  ...
  const gainersList: TokenChangeInfo[] = useMemo(() => {
    return gainers?.slice(0, 3).map((item: IGainer) => {
      ...
      return {
        ...
        change: {
          status: Number(priceChange) >= 0 ? MATH_NEGATIVE_TYPE.POSITIVE : MATH_NEGATIVE_TYPE.NEGATIVE,
  ...
  return (
    <GainerAndLoser
      gainers={gainersList}
      losers={loserList}
      loadingLose={isLoading || isLoadingListToken || isLoadingGetPoolList || isLoadingTokenDetail || isLoadingCommon}
      loadingGain={isLoading || isLoadingListToken || isLoadingGetPoolList || isLoadingTokenDetail || isLoadingCommon}
    />
  );
};
