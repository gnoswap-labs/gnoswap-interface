...
const LoadMoreButton: React.FC<LoadMoreProps> = ({ show, onClick, text = TEXT_TYPE.LOAD }) => {
  const { t } = useTranslation();

  return (
    <button type="button" onClick={onClick} css={wrapper}>
      <span>{t(show ? text[0] : text[1])}</span>
      {show ? <IconStrokeArrowDown className="icon-load" /> : <IconStrokeArrowUp className="icon-load" />}
    </button>
  );
};
