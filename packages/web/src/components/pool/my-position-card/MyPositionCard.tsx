...
const MyPositionCard: React.FC<MyPositionCardProps> = ({
  ...
  const isHideBar = useMemo(() => {
    const isAllReserveZeroBin40 = poolBin.every(item => Number(item.reserveTokenA) === 0 && Number(item.reserveTokenB) === 0);
    const isAllReserveZeroBin = positionBin.every(item => Number(item.reserveTokenA) === 0 && Number(item.reserveTokenB) === 0);
