...
export interface AccountNotificationRepository {
  ...
  createNotification: (address: string, transaction: TransactionModel) => Promise<boolean>;

  updateNotificationStatus: (address: string, txHash: string, status: StatusOptions) => Promise<boolean>;
