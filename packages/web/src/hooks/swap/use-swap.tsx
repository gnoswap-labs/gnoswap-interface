...
export const useSwap = ({ tokenA, tokenB, direction }: UseSwapProps) => {
  ...
  const [swapState, setSwapState] = useState<
    "NONE" | "LOADING" | "NO_LIQUIDITY" | "SUCCESS"
  >("NONE");
  ...
  const swap = useCallback(
    async (estimatedRoutes: EstimatedRoute[], tokenAmount: string) => {
      ...
      return await swapRouterRepository.swapRoute({
        inputToken: tokenA,
        outputToken: tokenB,
        estimatedRoutes,
        exactType: direction,
  ...
  const wrapToken = useCallback(
    async (token: TokenModel, tokenAmount: string) => {
      if (!account) {
        return false;
      }
      if (!selectedTokenPair) {
      ...
      return await swapRouterRepository.wrapToken({
        token,
  ...
  const unwrapToken = useCallback(
    async (token: TokenModel, tokenAmount: string) => {
      ...
      return await swapRouterRepository.unwrapToken({
        token,
        tokenAmount: Number(tokenAmount).toString(),
      });
    },
