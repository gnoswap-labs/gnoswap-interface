...
export const usePositionsRewards = ({ positions }: PositionsRewardsProps) => {
  ...
  const pooledTokenInfos = useMemo(() => {
    ...
    const pooledTokenAAmount = positions.reduce((accum: number | null, position) => {
    ...
    const pooledTokenBAmount = positions.reduce((accum: number | null, position) => {
    ...
    const tokenAPrice = tokenPrices[tokenAPriceId]?.usd ? Number(tokenPrices[tokenAPriceId]?.usd) : null;
    const tokenBPrice = tokenPrices[tokenBPriceId]?.usd ? Number(tokenPrices[tokenBPriceId]?.usd) : null;
    ...
    return [
      {
        ...
        amountUSD:
          pooledTokenAAmount !== null && tokenAPrice !== null
            ...
            : "-",
        rawAmountUsd: pooledTokenAAmount !== null && tokenAPrice != null ? pooledTokenAAmount * tokenAPrice : null,
      },
      {
        token: tokenB,
        ...
        amountUSD:
          pooledTokenBAmount !== null && tokenBPrice !== null
            ...
            : "-",
        rawAmountUsd: pooledTokenBAmount !== null && tokenBPrice != null ? pooledTokenBAmount * tokenBPrice : null,
      },
    ];
  }, [positions, tokenPrices]);

  const unclaimed: { rewards: RewardAccum[]; fees: RewardAccum[] } = useMemo(() => {
    if (positions.length === 0) {
      return { rewards: [], fees: [] };
    }
    ...
    const rewards = rewardTokenList.reduce<RewardAccum[]>((rewardList, rewardTokenPath) => {
      const rewardToken = tokens.find(item => item.path === rewardTokenPath);
      const tokenPriceId = checkGnotPath(rewardToken?.priceID || "");
      const tokenPrice = tokenPrices[tokenPriceId]?.usd ? Number(tokenPrices[tokenPriceId]?.usd) : 0;
          ...
          position.rewards.forEach(rewardInfo => {
            if (rewardInfo.rewardToken.path === rewardTokenPath) {
              if (rewardInfo.rewardToken.rewardType !== "SWAP_FEE") {
                ...
                newAccum.fee.usd += Number(rewardInfo.claimableUsd) || Number(rewardInfo.claimableAmount) * tokenPrice;
              }
            }
          });
