...
export const useOneClickStakingModal = ({
  ...
  const priceRangeInfo = useMemo(() => {
    ...
    if (
      !selectPool.maxPrice ||
      BigNumber(selectPool.maxPrice).isLessThan(selectPool.currentPrice)
    ) {
      inRange = false;
    }
    if (
      !selectPool.minPrice ||
      BigNumber(selectPool.minPrice).isGreaterThan(selectPool.currentPrice)
    ) {
      inRange = false;
    }
    return {
